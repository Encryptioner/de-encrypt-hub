...
import { Copy, Key, FileText, File as FileIcon, Shield, Download, Lock, Unlock } from 'lucide-react';
  ...
  const handleGenerateKeys = async () => {
    try {
      const keyPair = await window.crypto.subtle.generateKey(
        {
          name: 'RSA-OAEP', // Use OAEP for encryption compatibility
          modulusLength: 2048,
          publicExponent: new Uint8Array([0x01, 0x00, 0x01]),
          hash: 'SHA-256',
        },
        true,
        ['encrypt', 'decrypt', 'sign', 'verify'] // Add encrypt/decrypt permissions
      );
    ...
    <>
      <div className="space-y-6 pt-4">
       ...
       <p className="text-xs text-muted-foreground w-full text-center pt-6">
        Uses RSA-PSS for signatures and RSA-OAEP for encryption, both with SHA-256. Keys are in JWK format.
      </p>
    </>
  );
}
